...
async function uploadImageToDrive(base64Data, fileName, folderId) {
    try {
        ...
        const directUrl = `https://lh3.googleusercontent.com/d/${file.data.id}=s2000`;
...
async function uploadImages(images, clientName, leadId) {
    try {
        ...
        const photoUrls = {
            ...
            'Right Side': '', 'Back View': '', 'Other 1': '', 'Other 2': '', 'Other 3': ''
        };
        ...
        for (const img of images) {
            ...
            try {
                ...
                if (viewType === 'Top View') photoUrls['Top View'] = url;
                else if (viewType === 'Front View') photoUrls['Front View'] = url;
                else if (viewType === 'Left Side') photoUrls['Left Side'] = url;
                else if (viewType === 'Right Side') photoUrls['Right Side'] = url;
                else if (viewType === 'Back View') photoUrls['Back View'] = url;
                else if (otherCount <= 3) { photoUrls['Other ' + otherCount] = url; otherCount++; }
            } catch (imgError) {
        ...
        return { 'Top View': '', 'Front View': '', 'Left Side': '', 'Right Side': '', 'Back View': '', 'Other 1': '', 'Other 2': '', 'Other 3': '' };
    }
}

module.exports = { uploadImages, getOrCreateFolder, uploadImageToDrive };
